...
<ul class="nav nav-tabs">
  <li role="presentation"><a href="{{$apl_code_uri}}/appointment">申請受信一覧</a></li>
  <li role="presentation" class="active"><a href="{{$apl_code_uri}}/appointment/indexrequest">申請送信一覧</a></li>
</ul>
--}}
	...
	@if (count($appointments))
		@foreach($appointments as $appointment)
			<tr>
				@if ($appointment->fuser)
				<td><a href="{{$apl_code_uri}}/prof/{{$appointment->fuser->id}}"><img src="{{$appointment->fuser->avatar}}" width="50px" height="50px" /></a></td>
				<td>{{$appointment->fuser->name}}</td>
				@else
				<td></td>
				<td>ユーザが退会しました</td>
				@endif
				...
				<td><a href="{{$apl_code_uri}}/match/{{$appointment->match_id}}" class="btn btn-default btn-sm">詳細</a></td>
					@if ($appointment->status_id)
						@if ($appointment->status_id == \App\Appointment::STATUS_ID_REQUEST)
							<td class="warning">申請中</td>
							<td><a href="{{$apl_code_uri}}/appointmentcancel/{{$appointment->id}}/edit" class="btn btn-default btn-sm">確認/キャンセル</a></td>
						@elseif ($appointment->status_id == \App\Appointment::STATUS_ID_SUCCESS)
							<td class="success">成立</td>
							<td>
								<a href="{{$apl_code_uri}}/message/create/match_id/{{$appointment->match_id}}" class="btn btn-primary btn-sm">返信</a>
							</td>
						@elseif ($appointment->status_id == \App\Appointment::STATUS_ID_NG)
							<td class="danger">否認</td>
							<td><a href="{{$apl_code_uri}}/appointment/{{$appointment->id}}" class="btn btn-default btn-sm">確認
</a></td>
